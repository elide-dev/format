...
Local development helpers for the `elide-dev/format` action.
...
| `src/main.ts` | Orchestrator: option resolution, file discovery, formatter dispatch, outputs, summary |
| `src/options.ts` | Option types, defaults, normalization, input parsing |
| `src/command.ts` | Formatter invocation via argfiles, exit code handling |
