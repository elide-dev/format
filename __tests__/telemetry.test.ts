...
describe('telemetry', () => {
  ...
  it('should initialize Sentry when enabled', () => {
    const options = buildOptions({ formatter: 'ktfmt', mode: 'check' })
  ...
  it('should set action config tags on init', () => {
    const options = buildOptions({ formatter: 'ktfmt', mode: 'write' })
    initTelemetry(true, options)
    expect(setTagsMock).toHaveBeenCalledWith(
      expect.objectContaining({
        formatter: 'ktfmt',
        mode: 'write',
        action_version: expect.any(String)
      })
    )
  })
  ...
  it('logEvent should call captureMessage when enabled', () => {
    initTelemetry(true, buildOptions({}))
    logEvent('format.start', { installer: 'shell' })
    expect(captureMessageMock).toHaveBeenCalledWith('format.start', {
      level: 'info',
